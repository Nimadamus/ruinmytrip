...
/* The homepage asks Google, and a stranger, for people. Guides and the blog stay on their
   own URLs. They are not the front door. */
$people = strpos($home, 'Travelers with dates coming up');
ok('the people section exists', $people !== false);
ok('the homepage does not feature the guidebook', !str_contains($home, 'The boring bits, checked'));
ok('the homepage does not link the guide index', !str_contains($home, "url('guides')"));
ok('the homepage does not link the blog index', !str_contains($home, "url('blog')"));
