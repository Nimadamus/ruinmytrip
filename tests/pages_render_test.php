...
$routes = ['/', '/explore', '/travelers', '/going', '/meetups', '/buddies', '/buddies/cruise', '/buddies?where=Paris&from=2027-06-05&to=2027-06-12&flexible=1', '/buddies?where=Thailand&show=here', '/buddies?show=locals&interest=food&age=26-35', '/buddies/cruise?ship=Icon+of+the+Seas&line=Royal', '/buddies?where=nowhere-at-all&from=2027-01-01', '/talk', '/discover', '/reviews',
...
if ($acct) {
    ...
    $pdo->prepare("INSERT INTO buddy_posts (user_id,trip_type,title,where_text,date_from,date_to,spots,budget,description,status,created_at)
                   VALUES (?,'cruise','Render test cruise','Render ship from Miami',?,?,1,'any','Render test description text','open',?)")
...
if ($restore) {
    ...
    if (!empty($buddyId)) {
        ...
        ok('GET /buddy/{id} (signed out)', $st === 200 && str_contains($body, 'Render test cruise') && !str_contains($body, 'Render test note'),
           "status $st");
        // The results page with a real card on it, signed out, in both the plain and the cruise view.
        foreach (['/buddies', '/buddies/cruise?where=Render+ship', '/buddies?show=going&flexible=1'] as $path) {
            [$st, $body] = $req($path, null, '');
            $hit = '';
            foreach (BROKEN as $needle) if (str_contains($body, $needle)) { $hit = $needle; break; }
            ok("GET $path shows the card", $st === 200 && $hit === '' && str_contains($body, 'class="bcard') && str_contains($body, 'Render test cruise'),
               "status $st" . ($hit ? " contains $hit" : ''));
        }
