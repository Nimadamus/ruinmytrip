<?php /** @var string $qs @var array $dests @var array $trips @var array $guides @var array $reviews @var array $people */ ?>
<div class="wrap" style="min-height:50vh">
  ...
  <form action="<?= e(url('search')) ?>" method="get" style="display:flex;gap:10px;margin:14px 0 26px">
    <input type="search" name="q" value="<?= e($qs) ?>" placeholder="Destinations, trips, reviews, guides, travelers…" style="flex:1">
    <button class="btn btn-primary">Search</button>
  </form>
  <?php if ($qs===''): ?><p class="muted">Type a place, a trip, a review, or a traveler to begin.</p><?php else: ?>
    <?php if (!$dests && !$trips && !$reviews && !$guides && !$people): ?><p class="muted">No results for “<?= e($qs) ?>”.</p><?php endif; ?>
